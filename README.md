...
│   └── elements_page.py  # Базовые UI-элементы и формы
├── tests/                # Тесты по уровням
│   ├── api/
│   │   ├── conftest.py   # Фикстуры для тестирования API
│   │   └── test_users.py # Тесты REST API (CRUD, статус-коды)
│   └── ui/
│       ├── conftest.py   # Глобальные UI-фикстуры (браузер, сессии БД)
│       ├── test_demoblaze.py # E2E-сценарии с валидацией данных в SQL
│       └── test_elements.py  # UI-тесты (поиск, корзина с API-мокингом)
├── load_tests/           # Тестирование производительности
│   └── locustfile.py     # Скрипты нагрузочного тестирования в Locust
